...
- 已通过的完整阻断项仅有 Q34-B02 英文代表场景。软件键盘路径、ASS 固定帧以及多项性能子门槛通过，但不能替代完整阻断资格。
- 本轮确认两个可复现的物理故障：SSA 与 PGS 在字幕有效时段没有可见渲染；重启后 Playlist 条目仍在辅助功能树中，但条目列表可见高度为零，阻断安全作用域恢复闭环。
...
| Q34-P04 键盘与全屏 | `PASS` | Space、左右、上下、P 与 Control-Command-F 的软件键盘路径通过；真实硬件媒体键由 Q34-B01 单独判定。 |
...
| Q34-B04 字幕像素帧 | `FAIL` | ASS 固定帧通过；SSA 与 PGS 在字幕有效时段选择对应轨道后仍无可见字幕。 |
| Q34-B05 完整失败语料 | `FAIL` | 已覆盖六类失败输入；缺少与无效内容相区分的独立不支持编码样本。 |
| Q34-B06 场景元数据完整性 | `FAIL` | 仍有 FAIL、PARTIAL 与未执行项，不能支撑完整资格。 |
...
验收结束后已将 VoiceOver、外观与减弱动态效果恢复到初始值，退出候选与 QuickTime，并按备份哈希恢复用户 Playlist 数据库和偏好。原始证据不提交仓库；完整哈希索引、候选身份和恢复状态见最新脱敏 JSON。
