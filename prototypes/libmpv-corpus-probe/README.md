...
24 小时音频边界可加 `--long-audio-check`。探针会在载入后依次定位到 12 小时、
执行续播往返、定位到结尾前 2 秒并等待正常 EOF，然后退出。该模式只适用于
时长恰为 86,400 秒的专用生成样本。
...
`measure-energy-comparison.sh` 可按固定间隔记录同机对照的系统累计能耗计数、
目标进程 CPU/内存、系统负载、电池温度和热压状态。它不是 Apple 的逐进程
Energy Impact 指标；未取得管理员权限的环境只能把它用于相同输入、相同设备、
受控背景负载下的相对比较。
